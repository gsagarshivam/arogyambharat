...
if ($_POST['request'] == 1&&!empty($_POST['search'])) 
{
    ...
    $q = mysqli_query($conn,"SELECT client_id, m_name FROM customer_detail where client_id LIKE '%".$_POST['search']."%' ");
	if(mysqli_num_rows($q)!=0)
	{
...
if($_POST['request'] == 2)
{
    ...
    $q = mysqli_query($conn,"SELECT * FROM customer_detail WHERE client_id = '".$userid."'");
	if(mysqli_num_rows($q)!=0)
	{
	    while($row=mysqli_fetch_array($q)){
            $arr_result[] = array(
